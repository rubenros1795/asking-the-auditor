...
This is the exact annotation script used to produce
data/questions_llm_annotated.csv, reproduced here for methodological
transparency. Re-running it calls the Anthropic API and incurs cost; it is
NOT required to reproduce the paper's tables and figures (see ../analysis.ipynb).

Annotates questions with FEIT/CAU/OOR/ADV/? labels using Claude via the
Batches API (async, 50% cost vs. real-time). Reads and writes are relative
to ../data/ (see paths.py) and never overwrite the published corpus.

Usage
-----
    # Copy .env.example to .env and add your ANTHROPIC_API_KEY first.
...
If BATCH_ID is omitted the script reads it from state/batch_state.json,
which is written automatically by 'submit'.
...
from paths import QUESTIONS_SOURCE as INPUT_CSV, QUESTIONS_OUTPUT as OUTPUT_CSV, STATE_DIR
...
STATE_FILE       = STATE_DIR / "batch_state_questions.json"
STATE_FILE_RETRY = STATE_DIR / "batch_state_questions_retry.json"
