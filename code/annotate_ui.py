...
This is the manual annotation environment used to produce the reference
sample in data/questions_manual_annotated.csv and
data/answers_manual_annotated.csv (n=250 / n=251), reproduced here for
methodological transparency. It is NOT required to reproduce the paper's
tables and figures (see ../analysis.ipynb) — the finished manual labels are
already published in ../data/.

Two modes selectable from the sidebar:

  Vragen      Annotate questions with schema A (v0.7).
              Source  : data/questions_llm_annotated.csv
              State   : code/state/annotation_state_questions.json
              Output  : data/questions_manual_reannotated.csv

  Antwoorden  Annotate AR answers with schema B (v0.3)
              Source  : data/answers_llm_annotated.csv
              State   : code/state/annotation_state_answers.json
              Output  : data/answers_manual_reannotated.csv
...
from paths import (
    QUESTIONS_SOURCE,
    ANSWERS_SOURCE,
    QUESTIONS_MANUAL_OUTPUT,
    ANSWERS_MANUAL_OUTPUT,
    RAW_CSV,
    STATE_DIR,
)
...
STATE_Q = STATE_DIR / "annotation_state_questions.json"
STATE_A = STATE_DIR / "annotation_state_answers.json"
...
MODE_META = {
    "Vragen": {
        ...
        "state_file": STATE_Q,
        "output_csv": QUESTIONS_MANUAL_OUTPUT,
        "icon": "📋",
    },
    "Antwoorden": {
        ...
        "state_file": STATE_A,
        "output_csv": ANSWERS_MANUAL_OUTPUT,
        "icon": "💬",
    },
}
...
def _load_questions() -> pd.DataFrame:
    return pd.read_csv(QUESTIONS_SOURCE, sep="\t").reset_index(drop=True)


@st.cache_data
def _load_answers() -> pd.DataFrame:
    df = pd.read_csv(ANSWERS_SOURCE, sep="\t")
